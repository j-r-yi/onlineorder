...
public class AppConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                .csrf().disable()
                .authorizeHttpRequests(auth -> // defines permissions for different requests
                        auth
                                .requestMatchers(PathRequest.toStaticResources().atCommonLocations()).permitAll()
                                .requestMatchers(HttpMethod.GET, "/", "/index.html", "/*.json", "/*.png", "/static/**").permitAll() // frontend pages downloaded without authentication
                                .requestMatchers(HttpMethod.POST, "/login", "/logout", "/signup").permitAll() // login/logout/signup pages
        ...
        return http.build();  // a very large builder
    }
}
